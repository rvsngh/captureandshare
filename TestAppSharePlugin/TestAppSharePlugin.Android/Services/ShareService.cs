    ...
    public class ShareService : Activity, IShareService
    {
        public async void Share(string subject, string message, ImageSource imageSource)
        {
            ...
            try
            {
                ...
                intent.PutExtra(Intent.ExtraStream, FileProvider.GetUriForFile(CrossCurrentActivity.Current.Activity, "com.companyname.TestAppSharePlugin.fileprovider", path));
            }
            catch (Exception ex)
            {
